...
function getLiveStream($streamId)
{
	...
	foreach ($data as $item) {
		if (isset($item['stream_id']) && $item['stream_id'] == $streamId) {
			// Check for internal _stream_url first, then fall back to video_url and direct_source
			$urlParam = $item['_stream_url'] ?? $item['video_url'] ?? $item['direct_source'] ?? '';
	...
	// Handle i.mjh.nz streams - follow redirects and proxy the manifest
	if (stripos($urlParam, 'i.mjh.nz') !== false) {
		...
		if ($httpCode === 200 && !empty($content)) {
			// Serve the HLS manifest directly with proper content type
			...
			foreach ($lines as $line) {
				...
				// If it's a URL line (not a comment starting with #) and not already absolute
...
// Support both streamId and stream_id parameter names
$streamIdParam = $_GET['streamId'] ?? $_GET['stream_id'] ?? null;
getLiveStream($streamIdParam);
