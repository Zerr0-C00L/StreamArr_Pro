...
function fetchSeries($playVodUrl, $language, $apiKey, $totalPages)
{
    ...
    if (file_exists($syncScript)) {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            pclose(popen("start /B php \"$syncScript\" > NUL 2>&1", "r"));
        } else {
            exec("php \"$syncScript\" > /dev/null 2>&1 &");
        }
        error_log("Episode cache sync started in background");
    }
...
function isValidSeries($series) {
    ...
    // Exclude series with future first air dates
    $currentDate = time();
    $seriesAirTimestamp = strtotime($firstAirDate);
    
    if ($seriesAirTimestamp > $currentDate) {
        return false;
    }
