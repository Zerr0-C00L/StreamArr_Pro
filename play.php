...
// Continue processing even if client disconnects (prevents 499 from killing the cache update)
...
// Global to track successful hash for saving
$GLOBALS['successfulTorrentHash'] = null;
$GLOBALS['successfulTorrentFilename'] = null;
$GLOBALS['successfulTorrentBytes'] = 0;

/**
 * Save a successful torrent hash to the personal hashlist
 * This builds your own DMM-style hashlist over time
 */
function saveSuccessfulHash($imdbId, $type = 'movie', $season = null, $episode = null) {
    $hash = $GLOBALS['successfulTorrentHash'];
    $filename = $GLOBALS['successfulTorrentFilename'];
    $bytes = $GLOBALS['successfulTorrentBytes'];
    
    if (empty($hash)) return;
    
    try {
        $hashlist = new PersonalHashlists();
        $hashlist->addHash($hash, [
            'filename' => $filename,
            'bytes' => $bytes,
            'imdb_id' => $imdbId,
            'type' => $type,
        ...
        if ($GLOBALS['DEBUG']) {
            echo "✅ Saved hash to personal hashlist: $hash for $imdbId</br>";
        }
    } catch (Exception $e) {
        // Silent fail - don't break the stream
        if ($GLOBALS['DEBUG']) {
            echo "⚠️ Failed to save hash: " . $e->getMessage() . "</br>";
        }
    }
...
// Dev parameter - enable debug mode
...
// Handle direct hash playback (from source selection)
if (isset($_GET['hash']) && !empty($_GET['hash'])) {
    $hash = $_GET['hash'];
    $fileIdx = isset($_GET['fileIdx']) ? intval($_GET['fileIdx']) : 0;
    $fileName = isset($_GET['name']) ? $_GET['name'] : 'video.mkv';
    
    // Use Torrentio's resolve endpoint - it's much faster than doing RD API calls ourselves
    $torrentioResolveUrl = "https://torrentio.strem.fun/resolve/realdebrid/" 
        . urlencode($GLOBALS['PRIVATE_TOKEN']) 
        . "/" . urlencode($hash) 
        . "/null/" . $fileIdx 
        . "/" . urlencode($fileName);
    
    // Get the redirect location from Torrentio
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $torrentioResolveUrl);
    ...
    // Extract Location header (RD download URL)
    if (preg_match('/Location:\s*(.+)/i', $response, $matches)) {
        $downloadUrl = trim($matches[1]);
        
        // Extract the RD file ID from download URL to get streaming URL
        // Format: https://XX.download.real-debrid.com/d/FILEID23/filename
        // The streaming ID is FILEID (without the last 2 chars)
        if (preg_match('#/d/([A-Z0-9]+)/#i', $downloadUrl, $idMatch)) {
            $downloadId = $idMatch[1];
            // Remove last 2 characters to get streaming ID
            $rdFileId = substr($downloadId, 0, -2);
            
            // Get streaming transcode URL from RD API
            $streamApiUrl = "https://api.real-debrid.com/rest/1.0/streaming/transcode/" . $rdFileId 
                . "?auth_token=" . urlencode($GLOBALS['PRIVATE_TOKEN']);
            
            $streamResponse = @file_get_contents($streamApiUrl);
            if ($streamResponse) {
                $streamData = json_decode($streamResponse, true);
                
                // Prefer HLS for IPTV players, fallback to liveMP4
                if (!empty($streamData['apple']['full'])) {
                    $streamUrl = $streamData['apple']['full'];
                    header("Location: " . $streamUrl);
                    exit();
                } elseif (!empty($streamData['liveMP4']['full'])) {
                    $streamUrl = $streamData['liveMP4']['full'];
                    header("Location: " . $streamUrl);
                    exit();
                }
            }
        ...
        // Fallback to download URL with proxy
        if (isset($_GET['movieId'])) {
            writeToCache($_GET['movieId'] . '_tmdb_url', $downloadUrl);
        }
        redirectOrProxy($downloadUrl);
        exit();
    }
    
    // Fallback to our own RD resolution if Torrentio fails
    require_once 'libs/cached_sources.php';
    $sourceFinder = new CachedStreamSources($GLOBALS['PRIVATE_TOKEN']);
    $streamUrl = $sourceFinder->getStreamUrl($hash, $fileIdx);
    
    if ($streamUrl) {
        redirectOrProxy($streamUrl);
        exit();
    } else {
        http_response_code(404);
        echo "Failed to get stream URL for hash: " . htmlspecialchars($hash);
        exit();
    }
}
...
// ========== QUALITY/STREAM SELECTION PARAMETERS ==========
// stream_id: Play a specific cached stream by its ID (from SQLite streams table)
// quality: Play best stream matching this quality (4k, 1080p, 720p)
$selectedStreamId = $_GET['stream_id'] ?? null;
$selectedQuality = $_GET['quality'] ?? null;

// ========== TORRENTIO-ONLY FAST PATH ==========
// This bypasses all other sources and goes straight to Torrentio
// Much faster than the old multi-source approach

// Log all requests for debugging
$logFile = __DIR__ . '/logs/requests.log';
$logEntry = date('Y-m-d H:i:s') . " | " . ($_SERVER['REQUEST_URI'] ?? '') . " | UA: " . ($_SERVER['HTTP_USER_AGENT'] ?? 'none') . "\n";
@file_put_contents($logFile, $logEntry, FILE_APPEND);

/**
 * Fetch streams from a specific provider (Comet, MediaFusion, or Torrentio)
 * With fallback support for datacenter IP blocking
 */
function fetchStreamsFromProvider($provider, $imdbId, $type, $season, $episode, $rdKey, $debug = false) {
    $streams = [];
    $url = '';
    
    switch ($provider) {
        case 'comet':
            // Comet - works well with datacenter IPs (Hetzner, etc.)
            // Build config with RD key
            $cometConfig = [
        ...
        case 'mediafusion':
            // MediaFusion - also datacenter friendly
            // Uses a simpler config approach
            $mfConfig = [
                'streaming_provider' => [
                    'token' => $rdKey,
                    'service' => 'realdebrid'
    ...
    if ($debug) echo "Provider URL: " . substr($url, 0, 100) . "...</br>";
    ...
    // Check for Cloudflare block or error
    if ($httpCode !== 200 || strpos($response, 'Cloudflare') !== false || strpos($response, 'Attention Required') !== false) {
        if ($debug) echo "Provider $provider returned error (HTTP $httpCode) or Cloudflare block</br>";
        return [];
    }
    ...
    // Normalize stream format for different providers
    foreach ($streams as &$stream) {
        // Comet uses different field names - normalize them
        if ($provider === 'comet') {
            // Comet marks RD cached streams with [RD⚡] in name
            $name = $stream['name'] ?? '';
        ...
        // MediaFusion normalization
        if ($provider === 'mediafusion') {
            $name = $stream['name'] ?? '';
            // MediaFusion marks cached streams differently
            if (strpos($name, '⚡') !== false || strpos($name, 'RD') !== false) {
                if (strpos($name, '[RD+]') === false) {
                    $stream['name'] = '[RD+] ' . $name;
                }
            }
...
/**
 * Fast Torrentio playback using IMDB ID directly (for series where we have it)
 * @param string|null $streamId - Optional specific stream ID from SQLite cache
 * @param string|null $quality - Optional quality filter (4k, 2160p, 1080p, 720p, 480p)
 */
function torrentioFastPlayDirect($imdbId, $type, $season = null, $episode = null, $streamId = null, $quality = null) {
    $rdKey = $GLOBALS['PRIVATE_TOKEN'];
    $debug = $GLOBALS['DEBUG'] ?? false;
    
    // Log to file for debugging
    $logFile = __DIR__ . '/logs/requests.log';
    @file_put_contents($logFile, date('H:i:s') . " torrentioFastPlayDirect: $imdbId, $type, S$season E$episode, streamId=$streamId, quality=$quality\n", FILE_APPEND);
    
    if ($debug) echo "=== TORRENTIO DIRECT (IMDB: $imdbId) ===</br>";
    if ($debug && $streamId) echo "Stream ID: $streamId</br>";
    if ($debug && $quality) echo "Quality filter: $quality</br>";
    
    // ========== CHECK SQLITE FOR CACHED STREAM ==========
    // If we have a stream_id or quality, try SQLite first (with 24h TTL check)
    $streamCacheTTL = 24; // hours - RD cache status can change
    if ($streamId || $quality) {
        ...
        $cacheIsFresh = $db->hasValidStreams($imdbId, $dbType, $season, $episode, $streamCacheTTL);
        
        if ($cacheIsFresh) {
            // Get cached streams from SQLite
            if ($type === 'series' && $season !== null && $episode !== null) {
                $cachedStreams = $db->getStreams($imdbId, 'series', $season, $episode);
            } else {
                $cachedStreams = $db->getStreams($imdbId, 'movie');
            }
        } else {
            $cachedStreams = [];
            if ($debug) echo "Stream cache expired or missing (TTL: {$streamCacheTTL}h), will refresh</br>";
        }
        
        if (!empty($cachedStreams)) {
            if ($debug) echo "Found " . count($cachedStreams) . " cached streams in SQLite</br>";
            
            $targetStream = null;
            
            // If stream_id specified, find exact match
            if ($streamId) {
                foreach ($cachedStreams as $s) {
                    if ($s['id'] == $streamId || $s['hash'] == $streamId) {
                        $targetStream = $s;
                        break;
                    }
                }
            ...
            // If quality specified (or no exact stream_id match), filter by quality
            if (!$targetStream && $quality) {
                ...
                $pattern = $qualityPatterns[$quality] ?? '/\b' . preg_quote($quality, '/') . '\b/i';
                
                foreach ($cachedStreams as $s) {
                    $streamQuality = $s['quality'] ?? $s['title'] ?? '';
                    if (preg_match($pattern, $streamQuality)) {
                        $targetStream = $s;
                        break;
                    }
                }
            ...
            // If we found a target stream, resolve and play it
            if ($targetStream && !empty($targetStream['resolve_url'])) {
                if ($debug) echo "Using cached stream: " . ($targetStream['title'] ?? $targetStream['quality']) . "</br>";
                @file_put_contents($logFile, date('H:i:s') . " SQLITE HIT: Playing stream " . ($targetStream['id'] ?? 'unknown') . " quality=" . ($targetStream['quality'] ?? 'unknown') . "\n", FILE_APPEND);
                
                // Resolve the stream URL
                $resolveUrl = $targetStream['resolve_url'];
                ...
                if (preg_match('/Location:\s*(.+)/i', $resolveResponse, $matches)) {
                    ...
                    if (strpos($rdUrl, 'failed') === false) {
                        if ($debug) echo "Resolved to: $rdUrl</br>";
                        redirectOrProxy($rdUrl);
                        exit();
                    }
                }
                // If resolution failed, fall through to regular stream selection
                if ($debug) echo "SQLite stream resolution failed, falling back...</br>";
            }
        }
    }
    
    // ========== REGULAR STREAM SELECTION ==========
    // Check short-term cache for resolved RD URL (5 minutes)
    $cacheKeySuffix = $quality ? "_{$quality}" : ($streamId ? "_{$streamId}" : '');
    $cacheKey = "{$imdbId}_{$type}_{$season}_{$episode}{$cacheKeySuffix}";
    $rdUrlCache = __DIR__ . '/cache/rd_urls.json';
    $cachedUrls = [];
    if (file_exists($rdUrlCache)) {
        $cachedUrls = json_decode(file_get_contents($rdUrlCache), true) ?? [];
    }
    
    // Return cached URL if still valid (5 min TTL)
    if (isset($cachedUrls[$cacheKey]) && $cachedUrls[$cacheKey]['expires'] > time()) {
        $cachedUrl = $cachedUrls[$cacheKey]['url'];
        @file_put_contents($logFile, date('H:i:s') . " CACHE HIT: Using cached RD URL\n", FILE_APPEND);
        if ($debug) echo "Using cached RD URL</br>";
        redirectOrProxy($cachedUrl);
        exit();
    }
    
    // ========== MULTI-PROVIDER STREAM FETCHING ==========
    // Try providers in order: Comet -> MediaFusion -> Torrentio
    $providers = $GLOBALS['STREAM_PROVIDERS'] ?? ['comet', 'mediafusion', 'torrentio'];
    $streams = [];
    $usedProvider = '';
    
    foreach ($providers as $provider) {
        if ($debug) echo "Trying provider: $provider</br>";
        
        $providerStreams = fetchStreamsFromProvider($provider, $imdbId, $type, $season, $episode, $rdKey, $debug);
        
        if (!empty($providerStreams)) {
            $streams = $providerStreams;
            $usedProvider = $provider;
            if ($debug) echo "✅ Got " . count($streams) . " streams from $provider</br>";
            break;
        } else {
            if ($debug) echo "❌ No streams from $provider, trying next...</br>";
        }
    }
    
    if ($debug) echo "Found " . count($streams) . " streams from $usedProvider</br>";
    
    // ========== AUTO-CACHE STREAMS TO SQLITE ==========
    // Cache all available streams for future playlist generation with full names
    if (!empty($streams)) {
        try {
            if (!class_exists('EpisodeCacheDB')) {
                require_once __DIR__ . '/libs/episode_cache_db.php';
            }
            $cacheDb = new EpisodeCacheDB();
            $dbType = ($type === 'series') ? 'series' : 'movie';
            
            // Only cache RD+ streams (already cached on Real-Debrid)
            $streamsToCache = [];
            ...
            if (!empty($streamsToCache)) {
                $cacheDb->saveStreams($imdbId, $dbType, $streamsToCache, $season, $episode);
                if ($debug) echo "Cached " . count($streamsToCache) . " streams to SQLite</br>";
            }
        } catch (Exception $e) {
            // Silent fail - don't break playback
            if ($debug) echo "Stream cache error: " . $e->getMessage() . "</br>";
        }
    }
    ...
    if ($quality) {
        ...
        $qualityMatchPattern = $qualityPatterns[$quality] ?? '/\b' . preg_quote($quality, '/') . '\b/i';
    }
    ...
    foreach ($streams as $stream) {
        ...
        if (strpos($name, '[RD+]') === false) {
            if ($debug) echo "Skip non-cached: " . substr($name, 0, 50) . "</br>";
            continue;
        }
        ...
        // If quality is specified, only accept streams matching that quality
        if ($qualityMatchPattern) {
            if (!preg_match($qualityMatchPattern, $title) && !preg_match($qualityMatchPattern, $name)) {
                if ($debug) echo "Skip quality mismatch: " . substr($title, 0, 50) . "</br>";
                continue;
            }
            if ($debug) echo "Quality match: $quality</br>";
        }
        
        // Get the resolve URL
        $resolveUrl = $stream['url'] ?? null;
        if (!$resolveUrl) {
            if ($debug) echo "No URL in stream</br>";
            continue;
        }
        ...
        if (preg_match('/Location:\s*(.+)/i', $resolveResponse, $matches)) {
            ...
            // Skip failed streams (Torrentio returns failed video URL)
            if (strpos($rdUrl, 'failed') !== false) {
                if ($debug) echo "Skip failed stream</br>";
                continue;
            }
            
            // Verify RD URL actually works (HEAD request)
            $ch2 = curl_init($rdUrl);
            ...
            if ($httpCode == 404 || $httpCode == 410) {
                if ($debug) echo "Skip invalid RD URL (HTTP $httpCode)</br>";
                @file_put_contents($logFile, date('H:i:s') . " SKIP invalid RD URL: HTTP $httpCode\n", FILE_APPEND);
                continue;
            }
            
            if ($debug) echo "Got RD URL: $rdUrl</br>";
            
            // Log that we resolved to RD URL
            $logFile = __DIR__ . '/logs/requests.log';
            @file_put_contents($logFile, date('H:i:s') . " RESOLVED to RD: " . substr($rdUrl, 0, 100) . "...\n", FILE_APPEND);
            
            // Cache the resolved URL for 5 minutes (handles multiple requests from IPTV clients)
            $cachedUrls[$cacheKey] = [
                'url' => $rdUrl,
                'expires' => time() + 300 // 5 minutes
            ];
            // Cleanup old entries
            $cachedUrls = array_filter($cachedUrls, fn($v) => $v['expires'] > time());
            @file_put_contents($rdUrlCache, json_encode($cachedUrls));
            
            // Fallback to direct RD download URL (skip HLS - it's slower to start)
            redirectOrProxy($rdUrl);
            exit();
        }
        
        if ($debug) echo "Failed to resolve</br>";
    }
    
    if ($debug) echo "No valid streams found</br>";
    
    // Log the failure
    @file_put_contents($logFile, date('H:i:s') . " NO STREAMS: No RD-cached streams found for $imdbId\n", FILE_APPEND);
    
    return false;
}

/**
 * Show "Not Available" error - returns a small error video or redirects to error page
 */
function showNotAvailable($title = 'Content not available') {
    $logFile = __DIR__ . '/logs/requests.log';
    @file_put_contents($logFile, date('H:i:s') . " NOT AVAILABLE: $title\n", FILE_APPEND);
    
    // Option 1: Return a small "Not Available" video (best for IPTV apps)
    $errorVideo = __DIR__ . '/videos/not_available.mp4';
    if (file_exists($errorVideo)) {
        header('Content-Type: video/mp4');
        header('Content-Length: ' . filesize($errorVideo));
        readfile($errorVideo);
        exit();
    }
    
    // Option 2: Return HTTP 404 with message (some apps handle this better)
    header('HTTP/1.1 404 Not Found');
    header('Content-Type: text/plain');
    echo "Content not available on Real-Debrid. Try again later or choose different content.";
    exit();
}

function torrentioFastPlay($tmdbId, $type, $season = null, $episode = null, $streamId = null, $quality = null) {
    global $apiKey;
    $debug = $GLOBALS['DEBUG'] ?? false;
    
    if ($debug) echo "=== TORRENTIO FAST PATH (TMDB: $tmdbId) ===</br>";
    
    // Try to get IMDB ID from SQLite cache first (for movies)
    $imdbId = null;
    if ($type === 'movies') {
        try {
            require_once __DIR__ . '/libs/episode_cache_db.php';
            ...
            if ($movieData && !empty($movieData['imdb_id'])) {
                ...
                if ($debug) echo "IMDB ID from cache: $imdbId</br>";
            }
        } catch (Exception $e) {
    ...
    if (!$imdbId) {
        ...
        if (!$imdbId) {
            if ($debug) echo "No IMDB ID found</br>";
            return false;
        }
        
        if ($debug) echo "IMDB ID from TMDB API: $imdbId</br>";
        
        // Cache the IMDB ID in SQLite for next time (movies only)
        if ($type === 'movies') {
            try {
                if (!isset($cache)) {
                    require_once __DIR__ . '/libs/episode_cache_db.php';
                ...
                if ($debug) echo "Cached IMDB ID for movie $tmdbId</br>";
            } catch (Exception $e) {
                // Silent fail
            }
        }
    ...
    // Use the direct function with IMDB ID
    return torrentioFastPlayDirect($imdbId, $type, $season, $episode, $streamId, $quality);
}

// Try Torrentio fast path for movies
if ($type == 'movies') {
    if (torrentioFastPlay($movieId, 'movies', null, null, $selectedStreamId, $selectedQuality)) {
        exit();
    }
    // No RD-cached streams found - show error
    showNotAvailable("Movie ID $movieId not available on Real-Debrid");
}

// Try Torrentio fast path for series
if ($type == 'series') {
    // Parse episode data: format is "imdbId:tmdbId/season/X/episode/Y"
    if (!empty($episodeData) && strpos($episodeData, ':') !== false) {
        $episodeDataArr = explode(':', $episodeData);
        $imdbId = $episodeDataArr[0]; // e.g., tt0903747
        $subEpData = explode('/', $episodeDataArr[1]);
        $seriesId = $subEpData[0]; // TMDB ID
        $seasonNum = intval($subEpData[2]);
        $episodeNum = intval($subEpData[4]);
        
        if ($GLOBALS['DEBUG']) {
            echo "Series: IMDB=$imdbId, TMDB=$seriesId, S{$seasonNum}E{$episodeNum}</br>";
        }
        
        // Use IMDB ID directly - no need to look it up
        if (torrentioFastPlayDirect($imdbId, 'series', $seasonNum, $episodeNum, $selectedStreamId, $selectedQuality)) {
            exit();
        }
        
        // No RD-cached streams found - show error
        showNotAvailable("$imdbId S{$seasonNum}E{$episodeNum} not available on Real-Debrid");
    }
    // No valid episode data - show error
    showNotAvailable("Invalid episode data");
}

// ========== END TORRENTIO FAST PATH ==========
...
//Run series
	...
	// 1. With encoded data: /series/user/pass/id.BASE64DATA.mkv  
	// 2. Simple XC format: /series/user/pass/episode_id.mkv
	
	if (!empty($episodeData) && strpos($episodeData, ':') !== false) {
		// Original flow with episodeData from URL
		$episodeDataArr = explode(':', $episodeData);
		$subEpData = explode('/', $episodeDataArr[1]);
		...
		seriesDetails_TMDB($movieId, $apiKey, $useRealDebrid, $episodeDataArr);
	} else {
		// Simple XC format - look up episode from SQLite database
		$tmdbEpisodeId = $movieId;
		$dbFile = __DIR__ . "/cache/episodes.db";
		
		$episodeInfo = null;
		
		// Try SQLite database first (fast, no memory issues)
		if (file_exists($dbFile)) {
			try {
				$db = new SQLite3($dbFile, SQLITE3_OPEN_READONLY);
				$stmt = $db->prepare('SELECT series_id, season, episode, imdb_id FROM episodes WHERE episode_id = :id');
				$stmt->bindValue(':id', (int)$tmdbEpisodeId, SQLITE3_INTEGER);
				$result = $stmt->execute();
				...
				// Silently continue to TMDB fallback
			}
		}
		
		// If not in cache, try TMDB API lookup for the episode
		if (!$episodeInfo) {
			$tmdbEpUrl = "https://api.themoviedb.org/3/tv/episode/{$tmdbEpisodeId}?api_key={$apiKey}";
			$tmdbEpResponse = @file_get_contents($tmdbEpUrl);
			
			if ($tmdbEpResponse) {
				$tmdbEp = json_decode($tmdbEpResponse, true);
				if (isset($tmdbEp['show_id']) && isset($tmdbEp['season_number']) && isset($tmdbEp['episode_number'])) {
					// Found it! Get series external IDs for IMDB
					$seriesExtUrl = "https://api.themoviedb.org/3/tv/{$tmdbEp['show_id']}/external_ids?api_key={$apiKey}";
					$seriesExtResponse = @file_get_contents($seriesExtUrl);
					$seriesExt = json_decode($seriesExtResponse, true);
					$imdbId = $seriesExt['imdb_id'] ?? 'tt0000000';
					
					$episodeInfo = [
						'series_id' => (string)$tmdbEp['show_id'],
						'season' => $tmdbEp['season_number'],
						'episode' => $tmdbEp['episode_number'],
						'imdb_id' => $imdbId
					];
					
					// Save to SQLite database for future
					if (file_exists($dbFile)) {
						try {
							$db = new SQLite3($dbFile);
							$stmt = $db->prepare('INSERT OR REPLACE INTO episodes (episode_id, series_id, season, episode, imdb_id) VALUES (:ep_id, :series_id, :season, :episode, :imdb_id)');
							$stmt->bindValue(':ep_id', (int)$tmdbEpisodeId, SQLITE3_INTEGER);
							$stmt->bindValue(':series_id', (int)$episodeInfo['series_id'], SQLITE3_INTEGER);
							$stmt->bindValue(':season', (int)$episodeInfo['season'], SQLITE3_INTEGER);
							$stmt->bindValue(':episode', (int)$episodeInfo['episode'], SQLITE3_INTEGER);
		...
		if ($episodeInfo) {
			...
			$seasonNoPad = $episodeInfo["season"];
			$episodeNoPad = $episodeInfo["episode"];
			$imdbId = $episodeInfo["imdb_id"] ?? "tt0000000";
			
			// Set global variables like original flow
			$season = str_pad($seasonNoPad, 2, "0", STR_PAD_LEFT);
			$episode = str_pad($episodeNoPad, 2, "0", STR_PAD_LEFT);
			$episodeId = 's' . $season . 'e' . $episode;
			$seriesCode = $episodeId;
			$movieId = $seriesId; // Update movieId to series ID
			
			$episodeDataArr = [$imdbId, $seriesId . "/season/" . $seasonNoPad . "/episode/" . $episodeNoPad];
			
			if (stripos($userAgent, 'MXPlayer') !== false) {
				throttleMxPlayerRequests($seriesId);
			}
			
			seriesDetails_TMDB($seriesId, $apiKey, $useRealDebrid, $episodeDataArr);
		} else {
			http_response_code(404);
			echo "Episode not found in TMDB. Invalid episode ID: {$tmdbEpisodeId}";
			exit();
		}
	}
}
...
/**
 * Check if the current request is from an Apple TV client
 * Apple TV uses AppleCoreMedia or Lavf (FFmpeg) user agents
 */
function isAppleTVClient() {
    $ua = $_SERVER['HTTP_USER_AGENT'] ?? '';
    return (
        stripos($ua, 'AppleCoreMedia') !== false ||
        stripos($ua, 'Lavf') !== false ||
        stripos($ua, 'Apple TV') !== false
    );
}

/**
 * Check if the current request is from an IPTV client that needs proxy
 * These clients may not handle 302 redirects to external domains
 */
function needsProxy() {
    $ua = $_SERVER['HTTP_USER_AGENT'] ?? '';
    // IPTV clients that may have redirect issues
    return (
        isAppleTVClient() ||
        stripos($ua, 'Lavf') !== false ||
        stripos($ua, 'VLC') !== false ||
        stripos($ua, 'ExoPlayer') !== false ||
        stripos($ua, 'Dalvik') !== false ||  // Android apps
        stripos($ua, 'okhttp') !== false ||
        stripos($ua, 'stagefright') !== false ||
        stripos($ua, 'Chillio') !== false
    );
}

/**
 * Check if the URL is a Real-Debrid download URL
 */
function isRealDebridUrl($url) {
    return stripos($url, 'real-debrid.com') !== false || 
           stripos($url, 'download.real-debrid') !== false;
}

/**
 * Convert RD download URL to HLS streaming URL
 * Download URL format: https://XX.download.real-debrid.com/d/FILEID23/filename
 * Streaming ID is FILEID (without last 2 chars)
 */
function getRdStreamingUrl($downloadUrl) {
    if (preg_match('#/d/([A-Z0-9]+)/#i', $downloadUrl, $idMatch)) {
        $downloadId = $idMatch[1];
        // Remove last 2 characters to get streaming ID
        $rdFileId = substr($downloadId, 0, -2);
        ...
        if ($streamResponse) {
            ...
            // Prefer HLS for IPTV players
            if (!empty($streamData['apple']['full'])) {
                return $streamData['apple']['full'];
            } elseif (!empty($streamData['liveMP4']['full'])) {
                return $streamData['liveMP4']['full'];
            }
        }
    }
    return null;
...
/**
 * Redirect to URL or proxy the stream for clients that don't handle redirects well
 * Real-Debrid download URLs are redirected directly for fastest playback
 */
function redirectOrProxy($url) {
    // Log the redirect for debugging
    $logFile = __DIR__ . '/logs/requests.log';
    @file_put_contents($logFile, date('H:i:s') . " REDIRECT: " . substr($url, 0, 150) . "...\n", FILE_APPEND);
    
    // For RD download URLs, redirect directly - fastest playback
    if (isRealDebridUrl($url)) {
        @file_put_contents($logFile, date('H:i:s') . " RD URL detected, sending 302\n", FILE_APPEND);
    ...
    if (isAppleTVClient()) {
        // Use nginx X-Accel-Redirect to proxy the stream for Apple TV clients
        // This avoids the cross-domain redirect issue
        if (preg_match('#^https://([^/]+)/(.+)$#', $url, $matches)) {
            $host = $matches[1];
            $path = $matches[2];
            header("Content-Type: video/mp4");
            header("X-Accel-Redirect: /internal_proxy/$host/$path");
            exit();
        }
    }
    // Standard redirect for other clients
    header("HTTP/1.1 302 Found");
    header("Location: $url");
    exit();
}
                    ...
                    if ($result !== false) {
                        ...
                        if ($GLOBALS['DEBUG']) {
                            ...
                            if ($lCheck !== false) {
                                ...
                                // Save successful torrent hash to personal hashlist
                                if (!empty($GLOBALS['successfulTorrentHash'])) {
                                    saveSuccessfulHash($imdbId, 'movie');
                                }
                                exit();
                            }
                        } else {
                    ...
                    if ($result !== false) {
                        ...
                        if ($GLOBALS['DEBUG']) {
                            ...
                            if ($lCheck !== false) {
                                ...
                                // Save successful torrent hash to personal hashlist
                                if (!empty($GLOBALS['successfulTorrentHash'])) {
                                    saveSuccessfulHash($imdbId, 'series', $seasonNoPad, $episodeNoPad);
                                }
                                exit();
                            }
                        } else {
...
// Extract quality from filename (returns integer like 2160, 1080, 720, etc.)
function extractQualityFromFilename($filename) {
    $filename = str_ireplace("4K", "2160p", $filename);
    if (preg_match('/\b(2160|1080|720|480|360)p?\b/i', $filename, $matches)) {
        return intval($matches[1]);
    }
    return 0;
}

// Verify if actual file quality matches expected quality (with tolerance for claimed quality)
function verifyFileQuality($actualFilename, $expectedQuality, $torrentTitle) {
    global $DEBUG;
    
    $actualQuality = extractQualityFromFilename($actualFilename);
    $expectedQualityInt = intval(preg_replace('/[^0-9]/', '', $expectedQuality));
    
    // Check if this is a YTS/YIFY torrent claiming 4K - they ALWAYS mislabel 4K
    // Check both torrent title AND actual filename for YTS markers
    $isYtsTitle = (stripos($torrentTitle, 'YTS') !== false || stripos($torrentTitle, 'YIFY') !== false);
    $isYtsFile = (stripos($actualFilename, 'YTS') !== false || stripos($actualFilename, 'YIFY') !== false);
    $isYts = $isYtsTitle || $isYtsFile;
    
    if ($isYts && $expectedQualityInt >= 2160) {
        if ($DEBUG) {
            echo "🔍 Quality verification: Expected=$expectedQualityInt (YTS 4K - auto-reject)<br>";
            echo "   Torrent title: $torrentTitle<br>";
            echo "   Actual filename: $actualFilename<br>";
            echo "   YTS detected in: " . ($isYtsTitle ? "title " : "") . ($isYtsFile ? "filename" : "") . "<br>";
            echo "   ❌ REJECTED! YTS never has real 4K content<br>";
        }
        return false;
    }
    
    // Check if the actual file is HDR/DV content - reject it
    // Match patterns like: .DV. -DV- .DV] DV.P5 .HDR. -HDR- HDR10 DoVi etc.
    if (preg_match('/[.\-\s](DV|HDR|HDR10|HDR10\+|DoVi|DolbyVision|Dolby[\s\.]?Vision)[.\-\s\[\]]/i', $actualFilename)) {
        if ($DEBUG) {
            echo "🔍 HDR/DV detection in filename:<br>";
            echo "   Torrent title: $torrentTitle<br>";
            echo "   Actual filename: $actualFilename<br>";
            echo "   ❌ REJECTED! HDR/Dolby Vision content not supported<br>";
        }
        return false;
    }
    
    // Check if the actual file is REMUX - reject it (very large files)
    if (preg_match('/\b(REMUX|BluRay\.?Remux|BD\.?Remux)\b/i', $actualFilename)) {
        if ($DEBUG) {
            echo "🔍 REMUX detection in filename:<br>";
            echo "   Torrent title: $torrentTitle<br>";
            echo "   Actual filename: $actualFilename<br>";
            echo "   ❌ REJECTED! REMUX files are too large<br>";
        }
        return false;
    }
    
    if ($DEBUG) {
        echo "🔍 Quality verification: Expected=$expectedQualityInt, Actual=$actualQuality<br>";
        echo "   Torrent title: $torrentTitle<br>";
        echo "   Actual filename: $actualFilename<br>";
    }
    
    // If we can't detect quality, allow it
    if ($actualQuality == 0) {
        if ($DEBUG) echo "   ⚠️ Could not detect actual quality - allowing<br>";
        return true;
    }
    
    // If expected quality is 2160 but actual is much lower (1080 or less), reject
    if ($expectedQualityInt >= 2160 && $actualQuality <= 1080) {
        if ($DEBUG) echo "   ❌ MISLABELED! Claimed 4K but actual is $actualQuality<br>";
        return false;
    }
    
    // If expected quality is 1080 but actual is much lower (720 or less), reject  
    if ($expectedQualityInt >= 1080 && $actualQuality <= 720) {
        if ($DEBUG) echo "   ❌ MISLABELED! Claimed 1080p but actual is $actualQuality<br>";
        return false;
    }
    
    if ($DEBUG) echo "   ✅ Quality verified OK<br>";
    return true;
}
...
function instantAvailability_RD($torrents, $tSite) {
    ...
    foreach ($torrents as $index => $torrent) {
        ...
        if (!$video) {
            if ($DEBUG) echo "❌ No suitable video file found - deleting<br>";
            Stremio_rd_api("torrents/delete/$torrentId", [], 'DELETE');
            continue;
        }
        
        // Get the actual filename from the selected video file
        $actualFilename = $info['files'][$video['index']]['path'] ?? '';
        $expectedQuality = $torrent['quality'] ?? '';
        $torrentTitle = $torrent['extracted_title'] ?? $torrent['title_long'] ?? '';
        
        // Verify the actual file quality matches what the torrent claims
        if (!empty($expectedQuality) && !verifyFileQuality($actualFilename, $expectedQuality, $torrentTitle)) {
            if ($DEBUG) echo "❌ Quality mismatch - skipping mislabeled torrent<br>";
            Stremio_rd_api("torrents/delete/$torrentId", [], 'DELETE');
            continue;
        }
        ...
        if (!empty($info['links'])) {
            ...
            if (!empty($unrestricted['download'])) {
                ...
                // Save successful hash to personal hashlist
                $hash = $torrent['hash'] ?? $torrent['infoHash'] ?? null;
                if ($hash) {
                    $GLOBALS['successfulTorrentHash'] = strtolower($hash);
                    $GLOBALS['successfulTorrentFilename'] = $unrestricted['filename'] ?? ($torrent['extracted_title'] ?? 'Unknown');
                    $GLOBALS['successfulTorrentBytes'] = $unrestricted['filesize'] ?? 0;
                }
...
function sortTorrentsByQuality(&$torrents) {
    usort($torrents, function($a, $b) {
        ...
        // If tvpack status is the same, then sort by quality (highest first)
        $qualityA = intval(preg_replace('/[^0-9]/', '', $a['quality']));
        $qualityB = intval(preg_replace('/[^0-9]/', '', $b['quality']));

        if ($qualityA != $qualityB) {
            // Sort by quality in descending order (highest first)
            return $qualityB - $qualityA;
        }

        // Deprioritize YTS/YIFY torrents (they often mislabel quality)
        $titleA = $a['extracted_title'] ?? '';
        $titleB = $b['extracted_title'] ?? '';
        $isYtsA = (stripos($titleA, 'YTS') !== false || stripos($titleA, 'YIFY') !== false) ? 1 : 0;
        $isYtsB = (stripos($titleB, 'YTS') !== false || stripos($titleB, 'YIFY') !== false) ? 1 : 0;
        
        if ($isYtsA != $isYtsB) {
            // Non-YTS torrents first (they have more accurate quality labels)
            return $isYtsA - $isYtsB;
        }

        // If quality is the same, sort by size (largest first - bigger = better quality)
        $sizeA = isset($a['size']) ? parseSize($a['size']) : 0;
        $sizeB = isset($b['size']) ? parseSize($b['size']) : 0;

        return $sizeB - $sizeA;
    });
}

// Helper function to parse size strings like "1.5 GB", "800 MB" into bytes
function parseSize($sizeStr) {
    ...
    if (preg_match('/^([\d.]+)\s*(GB|MB|KB|TB|B)?/i', $sizeStr, $matches)) {
...
function selectHashByPreferences($torrents, $maxResolution, $tSite, $service)
{
    ...
    // === Filter out HDR, CAM, TS, HDTS, HDCAM, Telesync, REMUX ===
    $filteredOutCount = 0;
    $torrents = array_filter($torrents, function ($torrent) use (&$filteredOutCount) {
        $title = $torrent['extracted_title'] ?? '';
        
        // Filter out HDR content (HDR, HDR10, HDR10+, Dolby Vision, DV)
        // Match: .DV. -DV- DV.P5 .DV] HDR HDR10 HDR10+ DoVi DolbyVision etc.
        if (preg_match('/[.\-\s](DV|HDR|HDR10|HDR10\+|DoVi|DolbyVision|Dolby[\s\.]?Vision)[.\-\s\[\]]/i', $title)) {
            $filteredOutCount++;
            return false;
        }
        
        // Filter out REMUX (very large files, often 40-80GB)
        if (preg_match('/\b(REMUX|BluRay\.?Remux|BD\.?Remux)\b/i', $title)) {
            $filteredOutCount++;
            return false;
        }
        
        // Filter out CAM, TS, Telesync, HDTS, HDCAM (low quality recordings)
        if (preg_match('/\b(CAM|HDCAM|TS|HDTS|Telesync|TeleSync|CAMRip|CAM\.Rip|TSRip|TS\.Rip|SCR|SCREENER|DVDScr)\b/i', $title)) {
            $filteredOutCount++;
            return false;
        }
    ...
    if ($GLOBALS['DEBUG']) {
        echo "Total torrents filtered out (HDR/CAM/TS): " . $filteredOutCount . "</br></br>";
    }
    if (empty($torrents)) {
        if ($GLOBALS['DEBUG']) {
            echo "No torrents available after filtering.</br></br>";
        }
        return false;
    }
    ...
    // === RealDebrid: Try torrents in order (best quality first) ===
    // Note: Real Debrid has DISABLED the batch instantAvailability API (error 37)
    // So we must try each torrent individually - but we process quickly and limit attempts
    if ($useRealDebrid === true && $service === 'RealDebrid') {
        if ($GLOBALS['DEBUG']) {
            echo "🎬 RealDebrid: Processing " . count($sortedTorrents) . " torrents (sorted by quality)...</br></br>";
        }
        
        $attemptCount = 0;
        $maxAttempts = 10; // Try up to 10 torrents before giving up
        
        foreach ($sortedTorrents as $torrent) {
            if ($attemptCount >= $maxAttempts) {
                if ($GLOBALS['DEBUG']) {
                    echo "⚠️ Reached max attempts ($maxAttempts), stopping search</br></br>";
                }
                break;
            }
            
            if ($GLOBALS['DEBUG']) {
                echo "🔍 [" . ($attemptCount + 1) . "/$maxAttempts] Trying: " . ($torrent['extracted_title'] ?? $torrent['hash']) . "</br>";
            }
        ...
        if ($GLOBALS['DEBUG']) {
            echo "❌ No working cached torrent found after $attemptCount attempts</br></br>";
        }
        return false;
    }
...
{
    global $PRIVATE_TOKEN, $type, $seasonNoPad, $episodeNoPad, $DEBUG;

    if ($DEBUG) {
        echo 'Started running stremioSites </br></br>';
    }

    // Use Torrentio with RD config - only cached torrents
    $torrentioConfig = "providers=yts,eztv,rarbg,1337x,thepiratebay,kickasstorrents,torrentgalaxy,magnetdl,horriblesubs,nyaasi,tokyotosho,anidex|sort=qualitysize|debridoptions=nodownloadlinks,nocatalog|realdebrid=" . $PRIVATE_TOKEN;
    
    $suffix = ($type === 'series') ? "stream/series/$imdbId:$seasonNoPad:$episodeNoPad.json" : "stream/movie/$imdbId.json";
    $url = "https://torrentio.strem.fun/$torrentioConfig/$suffix";
    
    if ($DEBUG) echo "Fetching: $url</br></br>";
    ...
    if ($DEBUG) echo 'Streams found: ' . count($streams) . '</br></br>';
    
    // Quality filter - exclude REMUX, HDR, DV, 3D, CAM, SCR
    $excludePatterns = ['/\bREMUX\b/i', '/\bHDR\b/i', '/\bDV\b/i', '/\bDolby.?Vision\b/i', '/\b3D\b/i', '/\bCAM\b/i', '/\bSCR\b/i', '/\bTS\b/i'];
    
    foreach ($streams as $stream) {
        $title = $stream['title'] ?? '';
        $name = $stream['name'] ?? '';
        
        // Skip if not cached in RD
        if (strpos($name, '[RD+]') === false) {
            if ($DEBUG) echo "Skip non-cached: $name</br>";
            continue;
        }
        
        // Skip excluded quality types
        $excluded = false;
        foreach ($excludePatterns as $pattern) {
            if (preg_match($pattern, $title) || preg_match($pattern, $name)) {
                if ($DEBUG) echo "Skip excluded quality: $title</br>";
                $excluded = true;
                break;
            }
        }
        ...
        // Use the resolve URL directly from Torrentio response
        if (!empty($stream['url'])) {
            if ($DEBUG) echo "Using stream URL: " . $stream['url'] . "</br>";
            
            // Follow the resolve URL to get RD download link
            ...
            if (preg_match('/Location:\s*(.+)/i', $resolveResponse, $matches)) {
                $streamUrl = trim($matches[1]);
                if ($DEBUG) echo "Got RD URL: $streamUrl</br>";
                return $streamUrl;
            }
        }
    }
    
    if ($DEBUG) echo "No suitable streams found</br>";
    return false;
}
...
{
    // Disable URL caching for movie/series - let Chillio handle caching
    // Only skip actual URL values, allow status flags like _running_ and _failed_
    if (strpos($key, '_tmdb_url') !== false) {
        // Still allow status flags
        if ($value !== '_running_' && $value !== '_failed_') {
            return;
        }
    }
...
function readFromCache($key, $report=true)
{
    ...
    // Only skip actual URL values, allow status flags like _running_ and _failed_
    if (strpos($key, '_tmdb_url') !== false) {
        // Still need to check for _running_ status to prevent duplicate requests
        $cacheFilePath = 'cache.json';
        if (file_exists($cacheFilePath)) {
            $cacheData = json_decode(file_get_contents($cacheFilePath), true) ?: [];
            if (isset($cacheData[$key])) {
                $val = json_decode($cacheData[$key]['value'], true);
                // Only return status flags, not actual URLs
                if ($val === '_running_' || $val === '_failed_') {
                    return $val;
                }
            }
        }
